...
Evaluator failures cross the runner boundary as diagnostic schema v1: a stable producer phase/code and a redacted reason capped at 240 characters. Candidate, rule-pack, dependency, source, serialization, and unexpected-internal failures are distinguishable without exposing analyzer output, host paths, environment data, or candidate secrets. The consuming harness adds bounded process and schema-validation diagnostics and retains offline recovery artifacts.
